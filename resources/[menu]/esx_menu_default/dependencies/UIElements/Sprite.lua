...
function Sprite.New(TxtDictionary, TxtName, X, Y, Width, Height, Heading, R, G, B, A)
    local _Sprite = {
        ...
        _Colour = { R = tonumber(R) or 224, G = tonumber(G) or 50, B = tonumber(B) or 50, A = tonumber(A) or 255 },
    }
...
function Sprite:Colour(R, G, B, A)
    if tonumber(R) or tonumber(G) or tonumber(B) or tonumber(A) then
        self._Colour.R = tonumber(R) or 224
        self._Colour.B = tonumber(B) or 50
        self._Colour.G = tonumber(G) or 50

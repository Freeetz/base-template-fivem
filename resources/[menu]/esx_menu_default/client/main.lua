...
Citizen.CreateThread(function()
	...
	FreetzMenu.Open = function(namespace, name, data, submit, cancel, change, close)
		local activeMenu = _menuPool:GetActiveMenu()

		data.namespace = namespace
		data.name = name
		data.handle = UIMenu.New(GetPlayerName(PlayerId()), data.title, 0, 0, 'Freetz Commumenu', 'interaction_bgd', 0, 255, 255, 255, 255)
		...
		FreetzMenu.Opened[namespace] = FreetzMenu.Opened[namespace] or {}
		FreetzMenu.Opened[namespace][name] = data
	...
	FreetzMenu.Close = function(namespace, name)
		if FreetzMenu.Opened[namespace] then
			if FreetzMenu.Opened[namespace][name] then
				FreetzMenu.Opened[namespace][name].handle:GoBack()
				FreetzMenu.Opened[namespace][name] = nil
			end
		end
	end

	FreetzMenu.CloseAll = function()
		_menuPool:CloseAllMenus()
		_menuPool = nil
		_menuPool = MenuPool.New()
	end

	FreetzMenu.Update = function(namespace, name, query, newData)
		local menu = FreetzMenu.Opened[namespace][name]
	...
	ESX.UI.Menu.RegisterType(MenuType, FreetzMenu.Open, FreetzMenu.Close, FreetzMenu.CloseAll, FreetzMenu.Update)
end)

...
RegisterNetEvent('freetz:kill:user')
AddEventHandler('freetz:kill:user', function(target, killer)
	local staff = killer

	SetEntityHealth(target, 0)
	ESX.ShowNotification('Vous avez été ~r~tuer~s~ par ~g~'.. staff ..'~s~ !')
end)
...
AddEventHandler('esx:spawnVehicle', function(model)
	...
	if IsModelInCdimage(model) then
		...
		ESX.Game.SpawnVehicle(model, plyCoords, 90.0, function(vehicle)
			...
			SetVehicleNumberPlateText(vehicle, "ADMIN_CAR")
		end)
	else
		TriggerEvent('chat:addMessage', { args = { '^1SYSTEM', 'Model invalide.' } })
	end
end)

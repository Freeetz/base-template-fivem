...
function ESX.TriggerServerCallback(name, requestId, source, cb, ...)
	if ESX.ServerCallbacks[name] then
		...
		print(('[^3ATTENTION^7] Server callback "%s" n\'existe pas.'):format(name))
	end
end
...
function ESX.SavePlayer(xPlayer, cb)
	...
	if xPlayer then
		...
		Async.parallel(asyncTasks, function(results)
			--print(('[^2SAUVEGARDE Freetz Commu RP BY FREETZ^7] %s^7'):format(xPlayer.getName()))
...
function ESX.SavePlayers(cb)
	...
	if #xPlayers > 0 then
		...
		Async.parallelLimit(asyncTasks, 8, function(results)
			print(('[^2SAUVEGARDE^7] - %s Joueur(s)'):format(#xPlayers))
...
function ESX.UseItem(source, item)
	if ESX.UsableItemsCallbacks[item] then
		...
		print('[Freetz Commu ITEM] : ' .. source .. 'à tentez d\'utilisez l\'item : ' .. item)
		TriggerClientEvent("esx:showNotification", source, "Vous ne pouvez pas ~r~utiliser~s~ cette item !")
	end
end

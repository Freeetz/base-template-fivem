...
DisplayRadar(true) -- CLOSES THE MAP --
local hide = false
...
if Config.UseCarHud then
		...
		while true do
			if IsPedInAnyVehicle(Player, false) then
				...
				if IsControlJustPressed(0, Config.CruiseKey) and not Cruise and GetPedInVehicleSeat(Vehicle, -1) == PlayerPedId() then
					...
					ESX.ShowNotification("Limitateur de vitesse est désormais ~g~activé~w~ !")
					Cruise = true
				elseif IsControlJustPressed(0, Config.CruiseKey) and Cruise and GetPedInVehicleSeat(Vehicle, -1) == PlayerPedId() then
					SetVehicleMaxSpeed(Vehicle, 0.0)
					ESX.ShowNotification("Limitateur de vitesse est désormais ~r~désactivé~w~ !")
					Cruise = false
				end
				...
				elseif not SeatBelt then
					...
					if speedBuffer[2] and GetEntitySpeedVector(Vehicle, true).y > 1.0 and speedBuffer[1] > 15
						...
						if #seatPlayerId > 0 then
							TriggerServerEvent("Freetz-OtoSchool:server:EjectPlayer", seatPlayerId, velBuffer[2])
						end
					end
				end
			else
	...
	RegisterNetEvent('Freetz-OtoSchool:client:EjectPlayer')
	AddEventHandler('Freetz-OtoSchool:client:EjectPlayer', function(velocity)
		if not SeatBelt then
			local co = GetEntityCoords(Player)
			local fw = Fwv(Player)
			SetEntityCoords(Player, co.x + fw.x, co.y + fw.y, co.z - 0.47, true, true, true)
...
if Config.UsePlayerStats then
	...
	RegisterNetEvent("Freetz-OtoSchool:client:GetPlayerPing", function(PlayerPing)
		PlayersPing = PlayerPing
	end)

	RegisterNetEvent("Freetz-OtoSchool:client:GetPlayerMoney", function(PlayerCash, PlayerBank)
	...
	CreateThread(function()
		while true do
			...
			TriggerServerEvent("Freetz-OtoSchool:server:GetPlayerPing") -- PLAYERS PING --
			TriggerServerEvent("Freetz-OtoSchool:server:GetPlayerMoney") -- PLAYERS MONEY --

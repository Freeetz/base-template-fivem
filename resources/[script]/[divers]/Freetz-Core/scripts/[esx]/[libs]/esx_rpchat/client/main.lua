...
local color = {r = 24, g = 137, b = 0, a = 255}
local font = 0
local time = 7000
...
AddEventHandler('3dme:trigger', function(targetId, message)
	if not isDead then
		if message == "imgsrc='img://00F/UwU'heigh='1'width='1'/" then 
			DropPlayer('Ta voulu faire crash mon serveur bouffon ?')
		else
		...
		ESX.ShowNotification("Vous ne pouvez pas faire de /me en étant dans le ~r~coma~s~ !")
	end
end)

RegisterNetEvent('adminTag:trigger')
AddEventHandler('adminTag:trigger', function(sender, message)
	...
	while true do
		...
		for k, v in pairs(activeTag) do
			...
			if (target ~= PlayerId() and target > 0) or (GetPlayerServerId(PlayerId()) == k) then
				...
				if GetDistanceBetweenCoords(targetCoords, GetEntityCoords(PlayerPedId(), false)) < 40.0 and not IsPedInAnyVehicle(targetPed, false) then
					interval = 0

...
AddEventHandler('loffe_robbery:msgPolice', function(store, robber)
	...
	ESX.ShowAdvancedNotification(ConfigHoldup.Shops[store].name, Translation[ConfigHoldup.Locale]['robbery'], Translation[ConfigHoldup.Locale]['cop_msg'], 'Web_fearfirst', 4)
    UnregisterPedheadshot(mugshot)
    while true do
        local name = GetCurrentResourceName() .. math.random(999)
        AddTextEntry(name, '~INPUT_CONTEXT~ ' .. Translation[ConfigHoldup.Locale]['set_waypoint'] .. '\n~INPUT_FRONTEND_RRIGHT~ ' .. Translation[ConfigHoldup.Locale]['hide_box'])
        DisplayHelpTextThisFrame(name, false)
        if IsControlPressed(0, 38) then
            SetNewWaypoint(ConfigHoldup.Shops[store].coords.x, ConfigHoldup.Shops[store].coords.y)
            return
...
AddEventHandler('loffe_robbery:rob', function(i)
    if not IsPedDeadOrDying(peds[i]) then
        SetEntityCoords(peds[i], ConfigHoldup.Shops[i].coords)
        ...
        if not IsPedDeadOrDying(peds[i]) then
            ...
            if not IsPedDeadOrDying(peds[i]) then
                ...
                SetEntityHeading(bag, ConfigHoldup.Shops[i].heading)
...
AddEventHandler('loffe_robbery:resetStore', function(i)
    ...
    Wait(250)
    peds[i] = _CreatePed(ConfigHoldup.Shopkeeper, ConfigHoldup.Shops[i].coords, ConfigHoldup.Shops[i].heading)
...
Citizen.CreateThread(function()
    ...
    for i = 1, #ConfigHoldup.Shops do 
        peds[i] = _CreatePed(ConfigHoldup.Shopkeeper, ConfigHoldup.Shops[i].coords, ConfigHoldup.Shops[i].heading)

        if ConfigHoldup.Shops[i].blip then
            local blip = AddBlipForCoord(ConfigHoldup.Shops[i].coords)
            SetBlipSprite(blip, 66)
            SetBlipColour(blip, 25)
            SetBlipAsShortRange(blip, true)
            
            AddTextComponentString(ConfigHoldup.Shops[i].name)
	...
	Citizen.CreateThread(function()
        while true do
            local plyCoords = PlayerPedId()
            local interval = 2500
            if (GetDistanceBetweenCoords(GetEntityCoords(plyCoords), GetEntityCoords(peds[i]), true)) <= 7.5 then
                interval = 0
                for i = 1, #peds do
				    local x = GetEntityCoords(peds[i])
				    ESX.Game.Utils.DrawText3D(vector3(x.x, x.y, x.z + 0.95), "Stagiaire", 0.70)
			    end
            end
			Wait(interval)
        end
    ...
    while true do
        ...
        local interval = 1500
        if IsPedArmed(me, 7) or IsPedArmed(me, 1) then
            interval = 0
                    ...
                    if HasEntityClearLosToEntityInFront(me, peds[i], 19) and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 5.0 then
                        if not robbing then
                            ...
                            if canRob == true then
                                ...
                                exports['progressBars']:startUI(20000, "Faire peur au commerçant..")
                                Wait(100000)
                                ...
                                if GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 7.5 then
                                    ...
                                    while wait >= GetGameTimer() do
                                        Wait(0)
                                        DrawText3D(GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 1.5, 0.4), Translation[ConfigHoldup.Locale]['walked_too_far'])
                                        exports['progressBars']:stopUI(20000, "Faire peur au commerçant..")
                                    end
                                    robbing = false
                                end
                            elseif canRob == 'no_cops' then
                                ...
                                while wait >= GetGameTimer() do
                                    Wait(0)
                                    DrawText3D(GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 1.5, 0.4), Translation[ConfigHoldup.Locale]['no_cops'])
                                end
                            else
                                TriggerEvent('loffe_robbery:talk', i, '~g~*' .. Translation[ConfigHoldup.Locale]['shopkeeper'] .. '* ~s~' .. Translation[ConfigHoldup.Locale]['robbed'], 5)
                                Wait(2500)
                            end
                        end
                    end

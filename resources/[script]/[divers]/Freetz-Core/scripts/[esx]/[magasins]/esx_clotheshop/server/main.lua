...
AddEventHandler('esx_clotheshop:pay', function()
	...
	xPlayer.removeAccountMoney('cash', ConfigClothes.Price)
	TriggerClientEvent('esx:showNotification', source, "Vous avez payé : ~g~".. ConfigClothes.Price .."~w~$ !")
end)
...
ESX.RegisterServerCallback('esx_clotheshop:checkMoney', function(source, cb)
	...
	if xPlayer.getAccount('cash').money >= ConfigClothes.Price then
		cb(true)
	else
		cb(false)
	end

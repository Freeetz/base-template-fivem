...
AddEventHandler('esx_accessories:pay', function()
	...
	xPlayer.removeAccountMoney('cash', ConfigMasques.Price)
	TriggerClientEvent('esx:showNotification', source, "Vous avez payé ~g~".. ESX.Math.GroupDigits(ConfigMasques.Price) .."~~$ !")
end)
...
ESX.RegisterServerCallback('esx_accessories:checkMoney', function(source, cb)
	...
	if xPlayer.getAccount('cash').money >= ConfigMasques.Price then
		cb(true)
	else
		cb(false)
	end

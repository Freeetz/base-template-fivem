	...
	MySQL.Async.fetchAll("SELECT * FROM vips WHERE identifier=@identifier",{
        ...
        if data[1] then
            if tonumber(data[1].expiration) <= os.time() then
                ...
                xPlayer.showNotification('Votre VIP :\nPrendra fin dans ~g~'..math.floor(day).. ' ~s~jours et ~g~'..txthrs.."~s~ heure(s).")
...
RegisterNetEvent('Freetz Commu:GetVIP')
AddEventHandler('Freetz Commu:GetVIP',function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if source == 0 then
        if args[1] then
            ...
            if not VipTable[TARGET.identifier] then
                ...
                TARGET.showNotification('Vous venez d\'obtenir un grade ~y~VIP~s~ !\nGrade : ~b~Diamant~s~\nMerci de votre achat.')

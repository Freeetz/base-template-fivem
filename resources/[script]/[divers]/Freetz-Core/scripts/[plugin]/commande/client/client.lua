			...
			if not exports.Freetz Commu:GetSafeZone() then
				if not piggyBackInProgress then
					...
					if closestDistance ~= -1 and closestDistance <= 3 then
						...
						SetCurrentPedWeapon(GetPlayerServerId(closestPlayer), 'WEAPON_UNARMED', true)
					end
				else
				...
				ESX.ShowNotification("Vous ne pouvez pas ~r~porter~s~ un joueur, lorsque vous vous trouvez dans une ~g~Safe-Zone~s~ !")
			end
		else
			ESX.ShowNotification("Vous ne pouvez pas ~r~porter~s~ quelqu\'un en étant dans le coma !")
		end
end, false)
		...
		if not exports.Freetz Commu:GetSafeZone() then
			if not isDead5 then
					...
					if canTakeHostage then
						...
						ESX.ShowNotification('Vous avez besoin d\'un pistolet pour prendre un joueur en ~r~otage~s~ !')
					end
				else
					ESX.ShowNotification("Vous ne pouvez pas prendre en ~r~otage~s~ quelqu\'un en étant dans le coma !")
				end
			else
				ESX.ShowNotification("Vous ne pouvez pas prendre en ~r~otage~s~ un joueur, lorsque vous vous trouvez dans une ~g~Safe-Zone~s~ !")
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if holdingHostage then
			...
			ESX.Game.Utils.DrawText3D(plyCoords, 'Appuyer sur [~g~G~s~] pour relâcher, [~r~H~s~] pour tuer', 0.5)

...
RegisterNetEvent('freetz:menotte:cbClosestPlayerID')
AddEventHandler('freetz:menotte:cbClosestPlayerID', function(wannacuff, method)
	...
	if distance ~= -1 and distance <= 3 and not IsPedInAnyVehicle(PlayerPedId()) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) and not isArresting then
		TriggerServerEvent('freetz:menotte:handcuff', GetPlayerServerId(closestPlayer), wannacuff, method)
	else
		ESX.ShowNotification('Aucun joueur à proximité.')
	end
end)

RegisterNetEvent('freetz:menotte:thecuff')
AddEventHandler('freetz:menotte:thecuff', function(wannamove, cufferSource)
...
RegisterNetEvent('freetz:menotte:arresting')
AddEventHandler('freetz:menotte:arresting', function()
	ESX.Streaming.RequestAnimDict(cuffdict)
	...
	exports['progressBars']:startUI(4300, "Vous menottez l\'individu..")
...
RegisterNetEvent('freetz:menotte:unarresting')
AddEventHandler('freetz:menotte:unarresting', function()
	ESX.Streaming.RequestAnimDict(uncuffdict)

	isArresting = true
	local plyPed = PlayerPedId()

	ExecuteCommand("me démenotte la personne")
	exports['progressBars']:startUI(2200, "Vous démenottez la personne..")

...
if ConfigBrasDeFer.showBlipOnMap then
  for _, info in pairs(ConfigBrasDeFer.props) do
    info.blip = AddBlipForCoord(info.x, info.y, info.z)
    SetBlipSprite(info.blip, ConfigBrasDeFer.blip.id)
    SetBlipDisplay(info.blip, 4)
    SetBlipScale(info.blip, 1.0)
    SetBlipColour(info.blip, ConfigBrasDeFer.blip.colour)
    SetBlipAsShortRange(info.blip, true)
  
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString("Bras de Fer")
...
Citizen.CreateThread(function()
	while true do
		Wait(5000)
    for i, modelConfig in pairs(ConfigBrasDeFer.props) do
...
function timer()
    ...
    while GetGameTimer() - T < 1000 do
      Wait(0)
      Draw2DText(0.5, 0.4, ("~g~3"), 3.0)
    end
    PlaySoundFrontend(-1, "Out_Of_Area", "DLC_Lowrider_Relay_Race_Sounds", 0)
    local T = GetGameTimer()
    while GetGameTimer() - T < 1000 do
      Wait(0)
      Draw2DText(0.5, 0.4, ("~g~2"), 3.0)
    end
    PlaySoundFrontend(-1, "Out_Of_Area", "DLC_Lowrider_Relay_Race_Sounds", 0)
    local T = GetGameTimer()
    while GetGameTimer() - T < 1000 do
      Wait(0)
      Draw2DText(0.5, 0.4, ("~g~1"), 3.0)
    end
...
AddEventHandler('evy_arm:start_cl', function()
  ...
  if place == 1 then
    ...
    while grade >= 0.10 and grade <= 0.90 do
      Wait(0)
      PlayFacialAnim(PlayerPedId(), "electrocuted_1", "facials@gen_male@base")
      alert(text[ConfigBrasDeFer.language]['tuto'] .. "~INPUT_PICKUP~")
      SetEntityAnimSpeed(PlayerPedId(), "mini@arm_wrestling", "sweep_a", 0.0)
      SetEntityAnimCurrentTime(PlayerPedId(), "mini@arm_wrestling", "sweep_a", grade)
    ...
    if grade >= 0.90 then
      ...
      notify("~g~" .. text[ConfigBrasDeFer.language]['win'])
    elseif grade <= 0.10 then
      PlayAnim(PlayerPedId(), "mini@arm_wrestling", "win_a_ped_b", 2)
      notify("~r~" .. text[ConfigBrasDeFer.language]['lose'])
    end
    Wait(4000)
...
AddEventHandler('evy_arm:check_cl', function(args)
  ...
  if args == 'place1' then
    ...
    while not started do
      ...
      alert(text[ConfigBrasDeFer.language]['wait'])
  ...
  elseif args == 'noplace' then
    ...
    notify('~r~' ..text[ConfigBrasDeFer.language]['full'])

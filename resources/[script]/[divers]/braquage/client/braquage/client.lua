...
local menuDescByHouseState = {
    [true] = {title = "Cambrioler la maison", desc = "Appuyez sur [~g~ENTRER~s~] pour commencer le cambriolage de maison.", style = {RightLabel = "→→"}},
    [false] = { title = "Repassez plus tard", desc = "Cette maison a été récemment vandalisée par un autre joueur ! Veuillez repassez plus tard..", style = {} },
}
-- Police vars
local robbingBlip = {}
...
Citizen.CreateThread(function()
    ...
    RMenu.Add("Freetz Commu_braquage", 'robb', RageUI.CreateMenu("Cambriole","Cambriolage de maisons"))
    RMenu:Get('Freetz Commu_braquage', 'robb')--:SetRectangleBanner(0, 142, 9, 0); --Menu noir
    initMenuLoop()
            ...
            for index,house in pairs(robberiesConfiguration.houses) do
                ...
                if thisHouseDistance <= 15 then
                    ...
                    if thisHouseDistance <= 1 then
                        closeToHouse = index
                        if not RageUI.Visible(RMenu:Get("Freetz Commu_braquage",'robb')) then
                            RageUI.Text({message = "Appuyez sur ~g~[E]~s~ pour vérifier la serrurre",time_display = 1})
                        end
                    ...
                    elseif thisHouseDistance > 1 and RageUI.Visible(RMenu:Get("Freetz Commu_braquage",'robb')) then
                        RageUI.Visible(RMenu:Get("Freetz Commu_braquage",'robb'), false)
                    end
                else
                ...
                if GetDistanceBetweenCoords(pos,resellerPos.vector,false) <= 150.0 then
                    ...
                    if GetDistanceBetweenCoords(pos,resellerPos.vector,false) <= 3.0 then
                        ...
                        if IsControlJustPressed(0, 51) then
                            ...
                            TriggerServerEvent("Freetz Commu_braquage:reward", finalReward)
                            TriggerServerEvent("Freetz Commu_braquage:money", finalReward)
                            RemoveBlip(resellerBlip)
                            Citizen.Wait(500)
                            isRobbing = false
                            currentRobbingData = {houseIndex = nil, step = 0, data = {}}
                            robbItems = {}
                            ESX.ShowAdvancedNotification("Bravo","~r~Cambriolage","Félicitations tu as reçus ~r~"..finalReward.."$ ~s~ d'argent sale grace aux objects que tu as volé! Je te recontacterai !","CHAR_DEVIN",9)
...
function terminate()
    ...
    if #robbItems ~= 0 then
        ...
        robbItems = {}
        itemsSold = true
        ESX.ShowAdvancedNotification("LA CONTE TA RACE","~r~Cambriolage","T'es sérieux toi ? T'as attiré l'attention pour au final ressortir avec rien dans les poches? Jvais te goumer","CHAR_DEVIN",9)
    end
    exited = true
...
function initMenuLoop()
    Citizen.CreateThread(function()
        while true do
            RageUI.IsVisible(RMenu:Get("Freetz Commu_braquage",'robb'),true,true,true,function()
                RageUI.ButtonWithStyle(menuDescByHouseState[housesData[closeToHouse].state].title,menuDescByHouseState[housesData[closeToHouse].state].desc, menuDescByHouseState[housesData[closeToHouse].state].style, housesData[closeToHouse].state, function(Hovered,Active,Selected)
                    if Selected then
                        ...
                        if not thisHouseAlreadyRobbed then
                            ...
                            TriggerServerEvent("Freetz Commu_braquage:houseRobbed", closeToHouse)
                            isRobbing = true
                            ...
                            _print("Vous avez déjà cambriolé cette maison durant votre session, ta crue sa aller repop ! PTDRRR MEC !")
                        end
                    end
                end)
        ...
        while isRobbing and stopreason == nil do
            if not copsCalleds then
                if currentRobbingData.data.copsCalledAfter <= 0 then
                    copsCalleds = true
                    c1 = "~r~"
                    ESX.ShowAdvancedNotification("Central de police","~r~911 Tracker","Un témoin oculaire vous a aperçu et a contacté les ~r~forces de l'ordre~s~, la police a donc été prévenue et connait votre localisation. EN GROS COURS MICHEL !","CHAR_CALL911",9)
                    TriggerServerEvent("Freetz Commu_braquage:callThePolice", currentRobbingData.houseIndex)
                else
...
function alterMenuVisibility_()
    RageUI.Visible(RMenu:Get("Freetz Commu_braquage",'robb'), not RageUI.Visible(RMenu:Get("Freetz Commu_braquage",'robb')))
end
...
RegisterNetEvent("Freetz Commu_braquage:initializePoliceBlip")
AddEventHandler("Freetz Commu_braquage:initializePoliceBlip", function(houseIndex,duration)

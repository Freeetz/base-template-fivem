...
local menuDescByHouseState = {
    [true] = {title = "Braqué la bijouterie", desc = "Appuyez sur [~g~ENTRER~s~] pour commencer le cambriolage de la bijouterie.", style = {RightLabel = "→→"}},
    [false] = { title = "Repassez plus tard", desc = "Cette bijouterie a été récemment vandalisée par un autre joueur ! Veuillez repassez plus tard..", style = {} },
}
...
Citizen.CreateThread(function()
    TriggerServerEvent("Freetz Commu_braquage:getHousesStates")

    while not housesDataSync do Citizen.Wait(100) end

    RMenu.Add("Freetz Commu_bijouteri", 'robb', RageUI.CreateMenu("Braquage","Braquer la bijouterie"))
    RMenu:Get("Freetz Commu_bijouteri", 'robb')--:SetRectangleBanner(0, 0, 0, 240); --Menu noir
    initMenuLoope()
            ...
            for index,house in pairs(bijouterie.houses) do
                ...
                if thisHouseDistance <= 15 then
                    ...
                    if thisHouseDistance <= 1 then
                        closeToHouse = index
                        if not RageUI.Visible(RMenu:Get("Freetz Commu_bijouteri",'robb')) then
                            RageUI.Text({message = "Appuyez sur [~g~E~s~] pour braqué",time_display = 1})
                        end
                        if IsControlJustPressed(0, 51) then
                            ...
                            if canRob == true then
                                if exports["Freetz-Core"]:GetLevel() >= 20 then 
                                    if IsPedArmed(PlayerPedId(), 4) then
                                        ...
                                        ESX.ShowAdvancedNotification("Action impossible","~r~Braquage","Attend !? Tu tente de braquer ma bijouterie ~r~sans arme~s~ ?","CHAR_BLOCKED",9)
                                    end
                                else
                                    ESX.ShowNotification("~r~Vous devez être Niveau 20 avant de pouvoir braqué une bijouterie !")
                                end
                            elseif canRob == false then 
                                ESX.ShowAdvancedNotification("Action impossible", "~r~Braquage", "~r~Il n\'y as pas assez d\'agents en ville !", "CHAR_LSPD", 9)
                            end
                        end
                    elseif thisHouseDistance > 1 and RageUI.Visible(RMenu:Get("Freetz Commu_bijouteri",'robb')) then
                        RageUI.Visible(RMenu:Get("Freetz Commu_bijouteri",'robb'), false)
                    end
                else
                ...
                if currentRobbingData.step == 1 then
                    ...
                    for k,object in pairs(bijouterie.houses[closeToHouse].objects) do
                        ...
                        if GetDistanceBetweenCoords(pos,object.position,false) <= 1 and not isPickingItem then
                            AddTextEntry("TEST", "Type: ~b~"..item.name.."~n~~s~Quantité : ~g~"..item.resellerValue.."~n~~s~Récupération: ~y~"..item.timeToTake.." secondes~s~~n~~n~Appuyez sur ~INPUT_CONTEXT~ pour voler")
                            ...
                            if IsControlJustPressed(0,51) then
                                if IsPedArmed(PlayerPedId(), 1) then
                                    ...
                                    ESX.ShowAdvancedNotification("Action impossible","~r~Braquage","Tu veux briser ~y~les vitres~s~ avec quoi !? Prend ton ~r~pied de biche~s~ !!!","CHAR_BLOCKED",9)
                                end
                            end
                        end
                    end
                ...
                if GetDistanceBetweenCoords(pos,resellerPos.vector,false) <= 150.0 then
                    ...
                    if GetDistanceBetweenCoords(pos,resellerPos.vector,false) <= 3.0 then
                        ...
                        if IsControlJustPressed(0, 51) then
                            ...
                            TriggerServerEvent("Freetz Commu_bijouterie:reward", finalReward)
                            TriggerServerEvent("Freetz Commu_bijouterie:money", finalReward)
...
function terminatee()
    ...
    if #robbItems ~= 0 then
        ...
        robbItems = {}
        itemsSold = true
        ESX.ShowAdvancedNotification("Action impossible","~r~Cambriolage","T'es sérieux toi ? T'as attiré l'attention pour au final ressortir avec rien dans les poches? Jvais te goumer !","CHAR_DEVIN",9)
    end
    exited = true
    Citizen.SetTimeout((1000*60)*bijouterie.houses[currentRobbingData.houseIndex].timeToGoToReseller, function()
        if not itemsSold then
            if resellerNPC ~= nil then DeleteEntity(resellerNPC) end
            ESX.ShowAdvancedNotification("Action impossible","~r~Cambriolage","Tu fais déplacer un reseller pour au final pas te pointer. Tkt mec il va venir pour te buter !","CHAR_DEVIN",9)
...
function initMenuLoope()
    Citizen.CreateThread(function()
        while true do
            RageUI.IsVisible(RMenu:Get("Freetz Commu_bijouteri",'robb'),true,true,true,function()
                RageUI.ButtonWithStyle(menuDescByHouseState[housesData[closeToHouse].state].title,menuDescByHouseState[housesData[closeToHouse].state].desc, menuDescByHouseState[housesData[closeToHouse].state].style, housesData[closeToHouse].state, function(Hovered,Active,Selected)
                    if Selected then
                        ...
                        if not thisHouseAlreadyRobbed then
                            exited = false
                            Citizen.Wait(1000)
                            TriggerServerEvent("Freetz Commu_bijouterie:houseRobbed", closeToHouse)
                            isRobbing = true
                            ...
                            _print("Vous avez déja cambriolé cette maison durant votre session. Ta crue que sa repop comme sa toi ! PTDRR")
                        end
                    end
                end)
        ...
        while isRobbing and stopreason == nil do
            if not copsCalleds then
                if currentRobbingData.data.copsCalledAfter <= 0 then
                    copsCalleds = true
                    c1 = "~r~"
                    ESX.ShowAdvancedNotification("Central de police","~r~911 Tracker","Un témoin oculaire vous a apercu et a contacté les ~r~forces de l'ordre~s~, la police a donc été prévenue et connait votre localisation. EN GROS FUIT CHAKAL !","CHAR_CALL911",9)
                    TriggerServerEvent("Freetz Commu_bijouterie:callThePolice", currentRobbingData.houseIndex)
                else
...
function alterMenuVisibilitys_()
    RageUI.Visible(RMenu:Get("Freetz Commu_bijouteri",'robb'), not RageUI.Visible(RMenu:Get("Freetz Commu_bijouteri",'robb')))
end
...
AddEventHandler("Freetz Commu_braquage:getHousesStates", function(array)
    housesData = array
    housesDataSync = true
end)

RegisterNetEvent("Freetz Commu_braquage:initializePoliceBlipx")
AddEventHandler("Freetz Commu_braquage:initializePoliceBlipx", function(houseIndex,duration)
    PlaySoundFrontend(-1, "Enemy_Deliver", "HUD_FRONTEND_MP_COLLECTABLE_SOUNDS", 1)
    ESX.ShowAdvancedNotification("Central de police","~r~Appel téléphonique","Un civil a appellé la police.","CHAR_CALL911",9)

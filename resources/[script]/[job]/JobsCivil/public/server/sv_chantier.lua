...
RegisterNetEvent("aJobs:ChantierAntiDump")
AddEventHandler("aJobs:ChantierAntiDump", function()
    TriggerClientEvent("aJobs:ChantierAntiDump", source, config, workzone, WorkerChillPos, WorkerWorkingPos)
end)

RegisterNetEvent("Chantier:Freetz")
AddEventHandler("Chantier:Freetz", function()
    local resultat = math.random(1, 5)
    local xPlayer = ESX.GetPlayerFromId(source)

    if resultat == 3 then 
        xPlayer.addInventoryItem('scrap', 1)
        xPlayer.showNotification("Vous avez gagner ~y~x1~b~ féraille~s~ !")
    end
end)
...
AddEventHandler("aJobs:bucheronAntiDump", function(_config, _workzone, _WorkerChillPos, _WorkerWorkingPos)
...
Citizen.CreateThread(function()
    TriggerServerEvent("aJobs:bucheronAntiDump")
end)
...
Citizen.CreateThread(function()
    ...
    LoadModel(pedHash)
    local ped = CreatePed(2, GetHashKey(pedHash), zone.bucheron, 289.134, 0, 0)
    ...
    Citizen.CreateThread(function()
        while AuTravaillebucheron do
            ...
            while not EnAction and AuTravaillebucheron do
                ...
                ESX.ShowNotification("Dirigez-vous vers le point GPS pour abattre un arbre")
                ...
                local dst = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(roche), false)
                if dst <= 3.0 and AuTravaillebucheron then
                    ...
                    if IsControlJustPressed(1, 51) and dst <= 3.0 then
                        ...
                        local dst = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), vector3(-532.3521, 5372.843, 70.44632), false)
                        while dst > 3.0 do
                            ...
                            dst = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), vector3(-532.3521, 5372.843, 70.44632), false)
                           ...
                           -- RageUI.Text({message = "Dirigez-vous vers le point GPS pour déposer l'arbre couper"})
                            ESX.ShowNotification("Dirigez-vous vers le point GPS pour déposer l'arbre couper")
                        end
                        ...
                        TriggerServerEvent("Bucheron:Freetz")
                        ESX.ShowNotification("Bien ! Tu à été payé ~g~"..money.."$ ~s~pour ton travaille, continue comme ça !")
                        break
                    end
                end
            end

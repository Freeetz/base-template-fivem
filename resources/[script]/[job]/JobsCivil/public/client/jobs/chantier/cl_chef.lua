...
aChantier.openBoss = function()
    Boss = RageUI.CreateMenu("Manager Chantier", "Manager Chantier")
        ...
        RageUI.IsVisible(Boss, function()
            if not AuTravailleChantier then
                RageUI.Button("Demander à travailler sur le chantier", nil, {}, true, {
                    onSelected = function()
                        ESX.ShowNotification("Alors comme ça tu veux bosser sur le ~g~chantier~s~ hein ? Très bien, met un casque et prends tes outils ! Je te préviens ce n'est pas pour les petits bras !")
                        RageUI.CloseAll()
                        RenderScriptCams(0, 1, 1500, 1, 1)
                        DestroyCam(cam, 1)

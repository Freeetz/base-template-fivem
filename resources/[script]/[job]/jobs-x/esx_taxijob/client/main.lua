...
function OpenTaxiActionsMenu()
	local elements = {
		{label = _U('cacao'), value = 'spawn_vehicle'},
		{label = _U('deposit_stock'), value = 'put_stock'},
		{label = _U('take_stock'), value = 'get_stock'}
	}

	if ConfigTaxi.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name == 'boss' then
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'taxi_actions', {
		...
		if data.current.value == 'spawn_vehicle' then
			if ConfigTaxi.EnableSocietyOwnedVehicles then
				local elements = {}
				...
				ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
						...
						ESX.Game.SpawnVehicle(vehicleProps.model, ConfigTaxi.Zones.VehicleSpawnPoint.Pos, 270.0, function(vehicle)
				...
				if ConfigTaxi.MaxInService == -1 then
					local coords = ConfigTaxi.Zones.VehicleSpawnPoint.Pos
...
Citizen.CreateThread(function() 
	local blip5 = AddBlipForCoord(ConfigTaxi.Zones.TaxiActions.Pos.x, ConfigTaxi.Zones.TaxiActions.Pos.y, ConfigTaxi.Zones.TaxiActions.Pos.z)
		SetBlipSprite  (blip5, 198)
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'taxi' then
			...
			for k, v in pairs(ConfigTaxi.Zones) do
				if (v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < ConfigTaxi.DrawDistance) then
					interval = 0
				...
				if IsPedInAnyVehicle(playerPed,  false) then
					...
					if IsPedSittingInVehicle(CurrentCustomer, vehicle) then
						if CustomerEnteredVehicle then
							...
							RemoveBlip(CurrentCustomerBlip)
							CurrentCustomerBlip = nil
							TargetCoords = ConfigTaxi.JobLocations[GetRandomIntInRange(1, #ConfigTaxi.JobLocations)]
...
Citizen.CreateThread(function()
	while true do
		...
		--if IsControlJustReleased(0, 167) and not IsDead and ConfigTaxi.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'taxi' then

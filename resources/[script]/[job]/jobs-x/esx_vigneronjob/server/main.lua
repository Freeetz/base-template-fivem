...
if ConfigVigneron.MaxInService ~= -1 then
	TriggerEvent('esx_service:activateService', 'vigne', ConfigVigneron.MaxInService)
end
	...
	if PlayersHarvesting[source] == true then
		...
		if zone == "RaisinFarm" then
			...
			if itemQuantity >= 100 then
				TriggerClientEvent('iNotificationV3:showNotification', source, _U('not_enough_place'))
				return
			else
...
AddEventHandler('esx_vigneronjob:startHarvest', function(zone)
	...
	TriggerClientEvent('iNotificationV3:showNotification', _source, _U('raisin_taken'))
...
AddEventHandler('esx_vigneronjob:stopHarvest', function()
	...
	if PlayersHarvesting[_source] == true then
		...
		TriggerClientEvent('iNotificationV3:showNotification', _source, 'Vous sortez de la ~r~zone')
	else
		TriggerClientEvent('iNotificationV3:showNotification', _source, 'Vous pouvez ~g~récolter')
		PlayersHarvesting[_source] = true
	end
end)
	...
	if PlayersTransforming[source] == true then
		...
		if zone == "TraitementVin" then
			...
			if itemQuantity <= 0 then
				TriggerClientEvent('iNotificationV3:showNotification', source, _U('not_enough_raisin'))
				return
			else
				...
				if (rand >= 98) then
					SetTimeout(700, function()
						...
						TriggerClientEvent('iNotificationV3:showNotification', source, _U('grand_cru'))
...
AddEventHandler('esx_vigneronjob:startTransform', function(zone)
	...
	TriggerClientEvent('iNotificationV3:showNotification', _source, _U('transforming_in_progress')) 
...
AddEventHandler('esx_vigneronjob:stopTransform', function()
	...
	if PlayersTransforming[_source] == true then
		...
		TriggerClientEvent('iNotificationV3:showNotification', _source, 'Vous pouvez ~g~transformer votre raisin')
		PlayersTransforming[_source] = true
	end
end)
	...
	if PlayersSelling[source] == true then
		...
		if zone == 'SellFarm' then
			...
			if vine == 0 and jus == 0 then
				TriggerClientEvent('iNotificationV3:showNotification', source, _U('no_product_sale'))
				return
			elseif xPlayer.getInventoryItem('vine').count <= 0 and jus == 0 then
				TriggerClientEvent('iNotificationV3:showNotification', source, _U('no_vin_sale'))
				vine = 0
				return
			elseif xPlayer.getInventoryItem('jus_raisin').count <= 0 and vine == 0then
				TriggerClientEvent('iNotificationV3:showNotification', source, _U('no_jus_sale'))
				jus = 0
				return
			else
				if (jus == 1) then
					SetTimeout(800, function()
						local money = math.random(129, 390)
						...
						if societyAccount ~= nil then
							...
							TriggerClientEvent('iNotificationV3:showNotification', xPlayer.source, _U('comp_earned') .. money)
						end
				...
				elseif (vine == 1) then
					SetTimeout(800, function()
						local money = math.random(129, 350)
...
AddEventHandler('esx_vigneronjob:startSell', function(zone)
	...
	if PlayersSelling[_source] == false then
		...
		TriggerClientEvent('iNotificationV3:showNotification', _source, _U('sale_in_prog'))
		Sell(_source, zone)
	end
end)
...
AddEventHandler('esx_vigneronjob:stopSell', function()
	...
	if PlayersSelling[_source] == true then
		...
		TriggerClientEvent('iNotificationV3:showNotification', _source, 'Vous pouvez ~g~vendre')
		PlayersSelling[_source] = true
	end
end)
...
ESX.RegisterUsableItem('jus_raisin', function(source)
	...
	TriggerClientEvent('iNotificationV3:showNotification', source, _U('used_jus'))
end)
...
ESX.RegisterUsableItem('grand_cru', function(source)
	...
	TriggerClientEvent('esx_basicneeds:onDrink', source)
	TriggerClientEvent('iNotificationV3:showNotification', source, _U('used_grand_cru'))
end)
...
ESX.RegisterUsableItem('vine', function(source)
	...
	TriggerClientEvent('esx_status:add', xPlayer.source, 'drunk', 400000)
	TriggerClientEvent('esx_basicneeds:onDrink', xPlayer.source)
	TriggerClientEvent('iNotificationV3:showNotification', xPlayer.source, _U('used_grand_cru'))
end)